...
def plan() -> None:
    ...
    subprocess.run(
        ["sh", "-c", "tofu plan -out artifacts/plan.bin > artifacts/plan.log"]
    )
    subprocess.run(
...
def gen_modules() -> None:
    ...
    #     for m in MODULES_DIR.glob("module-*"):
    ...
    # # Create all modules
    # create_module_01_huge_single_file()
    # create_module_02_multiple_large_files()
    # create_module_03_many_tiny_files()
    # create_module_04_medium_complexity()
    # create_module_05_deep_nested()
    # create_module_06_data_heavy()
    # create_module_07_variable_explosion()
    # create_module_08_mixed_sizes()
    # create_module_09_submodules()
    # create_module_10_extreme()

    for m in MODULES_DIR.glob("module-*"):
        click.echo("Commiting module to own repository")
        push_module_to_github(m)
